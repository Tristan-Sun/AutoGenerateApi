...
def chineseEncode(content):
    return content.decode('gb2312').encode('UTF-8')

def chineseDecode(content):
    return content.decode('UTF-8').encode('gb2312')
...
DOMAIN = 'http://123.125.47.196:8888'
HOME_URL = DOMAIN + '/index.php'
# Url used to login.
LOGIN_URL = DOMAIN + "/api.php"
# REST api list.
API_URL = HOME_URL + chineseEncode('/APIÎÄµµ')
...
# Get API Lists
response, content = http.request(API_URL, method='GET', headers=headers)

apis = []
table = re.search(r'<table class="wikitable">(.*?)</table>', content, re.DOTALL)
currentGroup = None
currentDeveloper = None
for tr in re.findall(r'<tr.*?>.*?</tr>', table.group(1), re.DOTALL):
...
for api in apis:
    ...
    match = re.search(r'id="(API_Description|API_Intruduction|API_Introduction|\.E6\.8E\.A5\.E5\.8F\.A3\.E8\.AF\.B4\.E6\.98\.8E|\.E6\.8F\.8F\.E8\.BF\.B0)".*?<p>(?P<introduction>.+?)</p>.*?id="(Request_Method|\.E6\.8F\.90\.E4\.BA\.A4\.E6\.96\.B9\.E5\.BC\.8F)".*?<p>.*?<a.*?>http://api.m.renren.com/api/(?P<method>.*?)</a>.*?</p>.*?id="(Parameter_List|Request_Parameter[s]?_List|\.E8\.AF\.B7\.E6\.B1\.82\.E5\.8F\.82\.E6\.95\.B0)(\.E5\.88\.97\.E8\.A1\.A8)?(\.E7\.A4\.BA\.E4\.BE\.8B)?".*?<table class="wikitable".*?>(?P<table>.*?)</table>', content, re.DOTALL | re.IGNORECASE)
    # The order of introduction and method maybe changed, so we need to match for the second time.
    if not match:
        match = re.search(r'id="(API_Description|Request_Method|\.E6\.8F\.90\.E4\.BA\.A4\.E6\.96\.B9\.E5\.BC\.8F)".*?<p>.*?<a.*?>http://api.m.renren.com/api/(?P<method>.*?)</a>.*?</p>.*?id="(API_Intruduction|API_Introduction|\.E6\.8E\.A5\.E5\.8F\.A3\.E8\.AF\.B4\.E6\.98\.8E|\.E6\.8F\.8F\.E8\.BF\.B0)".*?<p>(?P<introduction>.+?)</p>.*?id="(Parameter_List|Request_Parameter[s]?_List|\.E8\.AF\.B7\.E6\.B1\.82\.E5\.8F\.82\.E6\.95\.B0)(\.E5\.88\.97\.E8\.A1\.A8)?(\.E7\.A4\.BA\.E4\.BE\.8B)?".*?<table class="wikitable".*?>(?P<table>.*?)</table>', content, re.DOTALL | re.IGNORECASE)
    if not match:
        print 'api %s not match...' % (api.name)
        continue

    api.introduction = chineseDncode(match.group('introduction').strip())
